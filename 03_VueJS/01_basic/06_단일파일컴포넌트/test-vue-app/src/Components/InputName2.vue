...
<template>
  <div>
    ...
    <!-- $emit 메소드를 사용하여 부모 컴포넌트로 이벤트 발신
     $emit 은 직계 부모 컴포넌트에게만 이벤트 발신 가능. 
      -->

    <!-- 버튼 클릿 시, NAMECHANGED 이벤트를 발신(방출)하고, 
       NAME 데이터가  들어간 변수를 전달-->
...
export default {
  ...
  // nameChanged 이벤트가 발생하는지 확인 -> nameChanged 이벤트 발신 허용, 다른 이벤트명은 경고 출력
  // emits: ['nameChanged1'],
  ...
  emits: {
    nameChanged: (e) => {
      ...
      // 이름이 문자열이고, 3글자 이상인 경우, 이벤트 발린 허용
      e.name && typeof e.name == 'String' && e.name.length >= 3;
    },
  },
  data() {
    return {
